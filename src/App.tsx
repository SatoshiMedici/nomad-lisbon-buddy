...
function TileMark({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <path d="M20 2 V11 M20 29 V38 M2 20 H11 M29 20 H38" stroke="var(--accent-soft)" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
  ...
  return (
    <div className="flex items-center gap-3 animate-fade-in-up">
      ...
      <div className="glass rounded-2xl rounded-tl-sm px-5 py-4 flex items-center gap-1.5">
        <span className="w-2 h-2 rounded-full bg-[var(--accent)]" style={{ animation: 'bounce-dot 1.2s infinite ease-in-out' }} />
        <span className="w-2 h-2 rounded-full bg-[var(--accent)]" style={{ animation: 'bounce-dot 1.2s infinite ease-in-out 0.2s' }} />
        <span className="w-2 h-2 rounded-full bg-[var(--accent)]" style={{ animation: 'bounce-dot 1.2s infinite ease-in-out 0.4s' }} />
      </div>
    </div>
  );
}
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-[rgba(247,246,244,0.82)] border-b border-[var(--border)]">
        <div className="max-w-4xl mx-auto px-5 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight leading-none">Nomad Lisbon Buddy</h1>
              <p className="text-xs text-[var(--muted)] mt-0.5">Your friendly local guide</p>
            </div>
          </div>
          {messages.length > 0 && (
            <button
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-5 sm:px-6 py-8 flex flex-col">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[var(--muted)] mt-2 text-base leading-relaxed max-w-2xl">
                Ask me anything about moving to and living in Lisbon — neighborhoods,
                visas, the NIF, banking, cafés with WiFi, transport, healthcare, housing,
                and the cost of living. I'll answer like a knowledgeable local friend over
                coffee.
              </p>
            </div>
          </div>
        ...
        <section className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-[var(--muted)]">Try one of these to start</p>
          </div>
              ...
              return (
                ...
                >
                  <span className="flex-shrink-0 w-9 h-9 rounded-xl bg-[var(--accent-glow)] flex items-center justify-center group-hover:bg-[var(--accent)] transition-colors">
                    <Icon className="w-4.5 h-4.5 text-[var(--accent)] group-hover:text-white transition-colors" style={{ width: 18, height: 18 }} />
                  </span>
                  <span className="text-sm font-medium leading-snug pt-1">{prompt.text}</span>
                </button>
              );
            })}
          </div>
        ...
        <section ref={threadRef} className="flex-1 space-y-5 mb-6 min-h-[120px]">
          {messages.length === 0 && !isThinking && (
            <div className="glass rounded-2xl p-6 text-center animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
              <p className="text-[var(--muted)] text-sm">
                Your conversation will appear here. Ask a question above or tap a suggestion
                to get started.
              </p>
            </div>
          )}
        ...
        <form onSubmit={handleSubmit} className="sticky bottom-0 -mx-5 sm:-mx-6 px-5 sm:px-6 pb-5 pt-3 bg-gradient-to-t from-[var(--bg)] via-[var(--bg)] to-transparent">
          ...
          <p className="text-xs text-[var(--muted)] mt-2 text-center">
            Friendly guidance, not official advice — verify legal and financial details
